<script lang="ts">
    ...
    $: if (reactiveTrigger || !reactiveTrigger) {
        if (filterByButton && window) {
            menuXPosition = filterByButton.getBoundingClientRect().x - filterByButtonWidth -  (menuWidth/2 + 35 /* Padding Right */)
            menuYPosition = filterByButton.getBoundingClientRect().y - filterByButtonHight/2 - menuHeight/2 
        }
    }
    ...
    onMount(() => {
        ...
        if (filterByButton) {
            menuXPosition = filterByButton.getBoundingClientRect().x -  (menuWidth/2 + 20 /* Padding Right */)
            menuYPosition = filterByButton.getBoundingClientRect().y + filterByButtonHight + 10 /* Padding Top  */
        }
...
<section data-container="wrapper">
    <main data-theme={theme}>
        ...
        <button bind:clientWidth={filterByButtonWidth} bind:clientHeight={filterByButtonHight} bind:this={filterByButton} data-button="filterby-button" use:clickoutside={{ enabled: true, callback: () => isFilterByButtonClicked = false }} on:click={toggleMenu}>
